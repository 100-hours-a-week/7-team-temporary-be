...
class AuthServiceTest {
    ...
    void 리프레시토큰_재발급에_성공하면_새_토큰을_반환한다() {
        ...
        given(jwtUtil.createAccessToken(any(), any(), any(), any(), any()))
                .willReturn("access-new");
        given(jwtUtil.createRefreshToken(any(), any(), any(), any(), any()))
                .willReturn("refresh-new");
    ...
    void 유효하지않은_토큰이면_로그아웃이_실패한다() {
        ...
        // when
        BaseException exception =
                assertThrows(BaseException.class, () -> authService.logout("access"));
