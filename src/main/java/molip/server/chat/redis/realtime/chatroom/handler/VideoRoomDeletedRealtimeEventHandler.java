...
public class VideoRoomDeletedRealtimeEventHandler implements ChatRoomRealtimeEventHandler {

    private static final String EVENT_TYPE = "video.room.deleted";
    ...
    public void handle(ChatRoomRealtimeEnvelope envelope) {
        try {
            VideoRoomDeletedResponse payload =
                    objectMapper.readValue(envelope.payloadJson(), VideoRoomDeletedResponse.class);

            log.info(
                    "broadcast room event: eventType={}, roomId={}", EVENT_TYPE, envelope.roomId());
